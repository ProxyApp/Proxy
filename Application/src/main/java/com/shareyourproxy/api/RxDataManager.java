...
public class RxDataManager {
    ...
    private RxDataManager(ProxyApplication app, SharedPreferences prefs, RxBusDriver bus) {
        _bus = bus;
        _app = app;
        _prefs = prefs;
    ...
    public static RxDataManager newInstance(
        ProxyApplication app, SharedPreferences prefs, RxBusDriver
        bus) {
        return new RxDataManager(app, prefs, bus);
    }
    ...
    private void userContactAddedEvent(UserContactAddedEventCallback event) {
        Message message =
            Message.create(UUID.randomUUID().toString(), event.user.id(), event.user.fullName());
